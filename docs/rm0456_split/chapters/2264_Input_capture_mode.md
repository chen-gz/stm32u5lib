...
from 00, the channel is configured in input and the TIMx_CCR1 register becomes readonly.
...
imagine that, when toggling, the input signal is not stable during at most five internal
...
Select the edge of the active transition on the tim_ti1 channel by writing the CC1P and
...
valid transition, so the prescaler is disabled (write IC1PS bits to 00 in the TIMx_CCMR1
register).
