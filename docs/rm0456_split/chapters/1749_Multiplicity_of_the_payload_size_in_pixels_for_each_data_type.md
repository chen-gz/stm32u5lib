...
DSI Host (DSI)
The LTDC interface can be configured to increase flexibility and promote its correct use for
several systems. The following configuration options are available:
•

Polarity control: all the control signals are programmable to change the polarity
depending on the LTDC configuration. It is recommended to keep the default polarity
configuration in both LTDC, DSI Host and DSI Wrapper, to guarantee a correct
behavior.
...
inferred as zero. The specification states that in this mode, the pixel line size must be a
multiple of four.
...
must occur for the command sampling and transmission. When shutting down the display, it
is necessary for the LTDC to be kept active for one frame after the command being issued.
...
pixels of payload is restricted to a multiple of a value, as shown in Table 438.
Table 438. Multiplicity of the payload size in pixels for each data type
Value
