...
Table 165. Data order in memory
Color mode
...
The output FIFO bytes are reordered to support display frame buffer update through a
...
When the byte swapping is activated (SB = 1 in DMA2D_OPFCCR), the number of pixel per
line (PL field in DMA2D_NLR) must be even, the output memory address (MA field in
DMA2D_OMAR) must be even, and the output line offset computed in bytes (resulting from
LOM field in DMA2D_CR and LO field in DMA2D_OOR values) must be even.
If not, a configuration error is detected.
Table 166. Standard data order in memory
Color Mode
