...
Square root
Table 208. Square root parameters
Parameter
...
The scaled value x · 2-n is programmed in ARG1 and the factor n must be programmed in
the SCALE parameter.
Table 209 lists the valid scaling factors, n, and the corresponding ranges of x and ARG1.
Table 209. Square root scaling factors and corresponding ranges
n
...
The primary result is the square root of x. RES1 must be multiplied by 2n to obtain the
correct value.
...
decimal places). The numeric range is therefore -1 (0x80000000) to 1 - 2-31 (0x7FFFFFFF).
In q1.15 format, the numeric range is 1 (0x8000) to 1 - 2-15 (0x7FFF). This format has the
...
Scaling factor
Several of the functions listed in Section 25.3.2 specify a scaling factor, SCALE. This allows
