...
impl PhyType {
    ...
    fn to_dspd(&self) -> stm32_metapac::otg::vals::Dspd {
        match self {
...
impl embassy_usb_driver::Driver<'_> for Driver {
    ...
    fn start(mut self, control_max_packet_size: u16) -> (Self::Bus, Self::ControlPipe) {
        let ep_out = self
            .alloc_endpoint(EndpointType::Control, control_max_packet_size, 0, Direction::Out)
            .unwrap();
        let ep_in = self
...
impl embassy_usb_driver::Endpoint for Endpoint {
    ...
    async fn wait_enabled(&mut self) {
        poll_fn(|cx| {
            ...
            unsafe { STATE.ep_in_wakers[ep_index].register(cx.waker()); }
