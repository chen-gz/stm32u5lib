...
pub fn setup_ov5640_camera<I2C: I2c<P>, P: Pin>(i2c: &mut I2C, power_down: &P, reset_b: &P) {
    ...
    i2c.write_read(OV5640_I2C_ADDR, &OV5640_CHIP_ID_HIGH_BYTE.to_be_bytes(), &mut read_val[0..1])
        .unwrap();
    i2c.write_read(OV5640_I2C_ADDR, &OV5640_CHIP_ID_LOW_BYTE.to_be_bytes(), &mut read_val[1..2])
        .unwrap();
    ...
    for &(reg, val) in ov5640_reg::OV5640_COMMON.iter() {
        ...
        i2c.write_retry(ov5640_reg::OV5640_I2C_ADDR, &reg_val, 5).unwrap();
    }
    ...
    i2c.write_retry(ov5640_reg::OV5640_I2C_ADDR, &write_val, 5).unwrap();
